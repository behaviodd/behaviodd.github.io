...
// 코어타임: 정오(12:00) ~ 17:00. 연차/반차/반반차를 쓰지 않는 한 반드시 근무.
export const CORE_START_MINUTES = 12 * HOUR; // 720
export const CORE_END_MINUTES = 17 * HOUR; // 1020

/**
 * 코어타임(12:00~17:00)을 반영해 실제 퇴근 시각을 보정한다.
 * 계산상 퇴근이 코어 종료(17:00)보다 이르면 17:00까지는 있어야 한다.
 */
export function applyCoreTime(leaveMinutes: number): {
  leave: number;
  floored: boolean;
} {
  if (leaveMinutes < CORE_END_MINUTES) {
    return { leave: CORE_END_MINUTES, floored: true };
  }
  return { leave: leaveMinutes, floored: false };
}
